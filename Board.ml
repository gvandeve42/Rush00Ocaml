...
(*                                                +#+#+#+#+#+   +#+           *)
(*   Created: 2018/10/07 22:30:34 by amerelo           #+#    #+#             *)
(*   Updated: 2018/10/07 23:25:32 by amerelo          ###   ########.fr       *)
(*                                                                            *)
(* ************************************************************************** *)
...
let rec isWinnerLine lgroup =  match lgroup with
	| [] -> Grid.Piece.E
	| v1::v2::v3::tail when (Grid.Piece.E != (get_w v1)) && (get_w v1) = (get_w v2) && (get_w v3) = (get_w v1) -> print_endline "find winner "; print_endline (Grid.piece_to_str (get_w v1))  ; get_w v1
...
let fix_x = [[" \\   /"]; ["   x  "]; [" /   \\"]]
let fix_o = [[" / - \\"]; [" |   |"]; [" \\ - /"]]
let get_fix p = if p = Grid.Piece.O then fix_o else fix_x

(* add generation of winner of depth n *)
let rec get_list_str board = match board with
	| Solo grid -> if (Grid.get_value_winner grid) <> Grid.Piece.E then get_fix  (Grid.get_value_winner grid) else (Grid.toStringList grid)
	| Group (listg, winner, depth) ->
										let newListStirng = let rec gen_lgrid listg i lst = match listg with 
														| [] -> lst
														| hd::tl -> gen_lgrid tl (i + 1) (merge_lst lst (get_list_str hd) depth i)
													in gen_lgrid listg 1 []
										in newListStirng

let rec can_i_play board  = match board with
	| Solo grid -> Grid.space grid
	| Group (listg, _ , _) -> 
					let rec loop listg = match listg with
							| [] -> false
							| hd::tl -> if (can_i_play hd) = true then true else loop tl
					in loop listg

let rec print_board board =
	let lst = get_list_str board in
